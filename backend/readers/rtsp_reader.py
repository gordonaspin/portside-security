...
class RTSPReader(Reader):
    ...
    def __init__(
        ...
        RTSPReader.segment_cleaner.add_folder(self.camera.segments_dir)

    @override
    def start(self):
        self._open_stream()
        self.reader_thread = Thread(target=self._frame_reader, daemon=True).start()
    ...
    def get_frame(self) -> NDArray[np.uint8] | None:
        ...
        try:
            ...
            self.camera.initialize_buffers(frame)
    ...
    def _open_stream(self):
        ...
        if not self.stop_event.is_set():
            ...
            filespec = os.path.join(self.camera.segments_dir, "%Y%m%d_%H%M%S.ts")
            ffmpeg_cmd = [
                "ffmpeg",
                ...
                "-i", self.camera.url,                   # RTSP stream from camera
                "-hide_banner",
                "-loglevel", "error",               # ONLY errors (no frame spam)
                "-nostats",
                
                "-filter_complex",                  # Split and reduce scale for raw only for OpenCV
                f"[0:v]scale={self.camera.width}:{self.camera.height},format=bgr24[raw]", # re-scale and raw BGR pixel format (OpenCV native)
    ...
    def _frame_reader(self):
        ...
        """
        current_thread().name = f"{self.camera.name} _frame_reader"

        frame_size = self.camera.width * self.camera.height * 3
        fail_count = 0
        ...
        while not self.stop_event.is_set():
            ...
            frame = np.frombuffer(raw, np.uint8).reshape((self.camera.height, self.camera.width, 3))

            # FPS calculation
            now = time.perf_counter()
            if self.camera.last_frame_time > 0:
                dt = now - self.camera.last_frame_time

                # filter pipeline artifacts
                if 0.02 < dt < 0.2:
                    inst_fps = 1.0 / dt
                    self.camera.dt.update(dt)
                    self.camera.fps.update(1.0 / self.camera.dt.value())

            self.camera.last_frame_time = now

            # latest-frame-wins
            if self.frame_queue.full():
                self.frame_queue.get_nowait()
                self.camera.total_drops += 1
            self.frame_queue.put(frame)
            self.camera.total_frames += 1
            self.camera.drop_rate = self.camera.total_drops / self.camera.total_frames
